...
// src/api/admin.ts
async function fetchAdminUsers(q = "", limit = 20, offset = 0) {
...
async function fetchAdminUserLoginHistory(userID, limit = 20) {
  return requestJson(
    `/api/admin/users/${encodeURIComponent(userID)}/login-history?limit=${limit}`
  );
}
async function updateAdminUserPassword(userID, newPassword) {
...
var require_admin = __commonJS({
  "src/admin.ts"() {
    ...
    var userList = byId("userList");
    var userCount = byId("userCount");
    var prevPageBtn = byId("prevPageBtn");
    var nextPageBtn = byId("nextPageBtn");
    var pageInfo = byId("pageInfo");
    var selectedUserTitle = byId("selectedUserTitle");
    var selectedUserMeta = byId("selectedUserMeta");
    var loginHistoryList = byId("loginHistoryList");
    var newPasswordInput = byId("newPassword");
    var confirmPasswordInput = byId("confirmPassword");
    var updatePasswordBtn = byId("updatePasswordBtn");
    var passwordStatus = byId("passwordStatus");
    var pageSize = 20;
    var query = "";
    var currentPage = 1;
    var totalUsers = 0;
    ...
    function formatLoginMethod(method) {
      if (method === "passkey") return t("admin.loginMethodPasskey");
      if (method === "register") return t("admin.loginMethodRegister");
    ...
    function renderLoginHistory(records) {
      ...
      loginHistoryList.innerHTML = records.map((record) => {
        ...
        return `
        <li>
          <div class="meta-title">${record.ip_address || t("admin.unknownIp")} \xB7 ${formatLoginMethod(record.login_method)} \xB7 ${formatDeviceType(record.device_type, (k) => k)}</div>
    ...
    async function selectUser(userID) {
      ...
      selectedUserMeta.textContent = `${user.email} \xB7 ${user.id}`;
