...
// src/api/chat.ts
async function sendAttachment(threadId, file) {
...
async function fetchChats(limit = 50) {
  return requestJson(`/api/chats?limit=${limit}`);
}
async function startChat(userId, llmConfigId) {
  return requestJson("/api/chats/start", {
    method: "POST",
    body: {
      user_id: userId,
      llm_config_id: llmConfigId && llmConfigId > 0 ? llmConfigId : void 0
    }
  });
}
async function fetchMessages(threadId, limit = 200, llmThreadId) {
...
async function fetchLLMThreads(threadId, activeThreadId) {
...
var require_chat = __commonJS({
  "src/chat.ts"() {
    ...
    var chatQuickStart = byId("chatQuickStart");
    var chatList = byId("chatList");
    var chatTitle = byId("chatTitle");
    var chatSubtitle = byId("chatSubtitle");
    var messageList = byId("messageList");
    var messageForm = byId("messageForm");
    var messageInput = byId("messageInput");
    var chatRefreshBtn = byId("chatRefreshBtn");
    var chatNewTopicBtn = byId("chatNewTopicBtn");
    var chatRenameTopicBtn = byId("chatRenameTopicBtn");
    var chatThreadBar = byId("chatThreadBar");
    var chatThreadSelect = byId("chatThreadSelect");
    var chatModelBar = byId("chatModelBar");
    var chatModelCurrent = byId("chatModelCurrent");
    var chatModelSelect = byId("chatModelSelect");
    var chatSwitchModelBtn = byId("chatSwitchModelBtn");
    var attachmentBtn = byId("attachmentBtn");
    var attachmentInput = byId("attachmentInput");
    var currentUserId = "";
    ...
    var activeMessages = [];
    var activeMessageLoadedAt = "";
    var activeLLMThreadId = null;
    var activeLLMThreads = [];
    var activeIsAIChat = false;
    var activeIsBotChat = false;
    var currentLLMConfigs = [];
    var activeChatSummary = null;
    var activeChatBlocked = false;
    var activeChatBlockMessage = "";
    var activeChatReplyRequired = false;
    var activeChatReplyRequiredMessage = "";
    var expandedMarkdownMessages = /* @__PURE__ */ new Set();
    var sharedMarkdownContentCache = /* @__PURE__ */ new Map();
    var sharedMarkdownLoading = /* @__PURE__ */ new Set();
    var messageCacheMap = /* @__PURE__ */ new Map();
    var MSG_PAGE_SIZE = 50;
    var visibleOlderCount = 0;
    var quickStartTargets = [];
    var quickStartLLMOptions = [];
    function getCacheKey() {
      return activeLLMThreadId != null ? `${activeThreadId}:${activeLLMThreadId}` : activeThreadId || "";
    }
    ...
    function attachmentIcon(mimeType) {
      ...
      if (mimeType.includes("zip") || mimeType.includes("compress")) return "\u{1F5DC}\uFE0F";
      return "\u{1F4CE}";
    }
  ...
  `;
    ...
    function updateActiveChatHeader() {
      ...
      chatSubtitle.textContent = activeChatBlocked && activeChatBlockMessage ? activeChatBlockMessage : activeChatReplyRequired && activeChatReplyRequiredMessage ? activeChatReplyRequiredMessage : formatPresence(chat);
    }
    ...
    function renderLLMThreadBar() {
      ...
      chatThreadSelect.innerHTML = activeLLMThreads.map((thread) => `<option value="${thread.id}">${escapeHtml(thread.title || t("chat.newTopic"))}</option>`).join("");
    ...
    function renderLLMThreadModelBar() {
      ...
      chatModelCurrent.textContent = activeThread.config_name ? `${activeThread.config_name}${activeThread.config_model ? ` \xB7 ${activeThread.config_model}` : ""}` : t("chat.followBotDefault");
      if (!currentLLMConfigs.length) {
        chatModelSelect.innerHTML = `<option value="">${t("chat.noConfigs")}</option>`;
        chatSwitchModelBtn.disabled = true;
        return;
      }
      chatModelSelect.innerHTML = currentLLMConfigs.map((config) => `<option value="${config.id}">${escapeHtml(config.name)} \xB7 ${escapeHtml(config.model)}</option>`).join("");
    ...
    function renderChatList(chats) {
      ...
      chats.forEach((chat) => {
        ...
        const unreadBadge = chat.unread_count ? `<span class="unread-badge">${chat.unread_count}</span>` : "";
    ...
    function renderQuickStart(targets, llmOptions) {
      ...
      const botOptionsHTML = targets.map((item) => `<option value="${escapeHtml(item.user_id)}">${escapeHtml(item.name)} \xB7 ${escapeHtml(item.meta)}</option>`).join("");
      const llmOptionsHTML = [
        `<option value="">\u81EA\u52A8\u9009\u62E9\uFF08\u4E0D\u9009\u4E5F\u80FD\u5F00\u59CB\uFF09</option>`,
        ...llmOptions.map((item) => `<option value="${item.id}">${escapeHtml(item.name)} \xB7 ${escapeHtml(item.model)}</option>`)
      ].join("");
  ...
  `;
    ...
    function renderMessages(messages, scrollToBottom = false) {
      ...
      const loadOlderHtml = visibleStart > 0 ? `<div class="message-load-older"><button class="btn-inline btn-secondary" id="msgLoadOlderBtn" type="button">\u2191 Load older messages (${visibleStart} more)</button></div>` : "";
      const prevScrollHeight = messageList.scrollHeight;
      const prevScrollTop = messageList.scrollTop;
      messageList.innerHTML = loadOlderHtml + visibleMessages.map((msg) => {
        ...
        const retryAction = isFailedBotReply ? `<button class="btn-inline btn-secondary message-retry" data-id="${msg.id}" type="button">Retry</button>` : "";
        const failureBadge = isFailedBotReply ? `<div class="message-failure-badge">${t("chat.sendFailed")}</div>` : "";
        const markdownActions = isSharedMarkdown ? `
          ...
          ` : "";
        const textActions = !isSharedMarkdown && retryAction ? `<div class="message-inline-actions">${retryAction}</div>` : "";
        const isAttachment = msg.message_type === "attachment" && Boolean(msg.attachment);
        const content = msg.deleted ? t("chat.messageRevoked") : isAttachment && msg.attachment ? renderAttachment(msg.attachment) : isSharedMarkdown ? `
            ...
            ` : isSystem ? renderMarkdown(msg.content || "") : escapeHtml(msg.content || "");
        const bubbleClass = msg.deleted ? "message-bubble deleted" : "message-bubble";
        const contentClass = isSharedMarkdown ? "message-bubble-content" : isSystem && !msg.deleted ? "message-bubble-content markdown-body" : "message-bubble-content";
        const revokeButton = isMine && !msg.deleted ? `<button class="message-revoke" data-id="${msg.id}" type="button">${t("chat.revoke")}</button>` : "";
        ...
        const botModelMeta = isBotReply ? activeThread?.config_model || activeThread?.config_name || "LLM" : "";
        const messageMeta = isBotReply ? `${msg.sender_username} \xB7 ${botModelMeta} \xB7 ${formatTime(msg.created_at)}${isFailedBotReply ? ` \xB7 ${t("chat.failed")}` : ""}` : `${msg.sender_username} \xB7 ${formatTime(msg.created_at)}`;
        if (isMine) {
          return `
      ...
      document.getElementById("msgLoadOlderBtn")?.addEventListener("click", () => {
      ...
      messageList.querySelectorAll(".message-revoke").forEach((button) => {
        button.addEventListener("click", async () => {
          ...
          await revokeMessage2(messageId);
        });
      });
    ...
    async function revokeMessage2(messageId) {
      if (!activeThreadId) {
        return;
      }
      const response = await revokeMessage(activeThreadId, messageId);
      if (!response.ok) {
        return;
      }
    ...
    function sendPresence(action, threadId) {
      ...
      ws.send(JSON.stringify({
        type: "presence",
        action,
        thread_id: parsedThreadId > 0 ? parsedThreadId : void 0
      }));
    }
      ...
      ws.addEventListener("message", async (event) => {
        ...
        if (payload.type === "revoke") {
          if (activeThreadId === chatId && chatId && payload.message_id) {
            const handled = payload.user_id === "retry" ? removeMessageIfNeeded(payload.message_id) : markMessageRevoked(payload.message_id);
            if (handled) {
              renderMessages(activeMessages);
            } else {
