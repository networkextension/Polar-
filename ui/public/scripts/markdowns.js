...
// src/markdowns.ts
var require_markdowns = __commonJS({
  "src/markdowns.ts"() {
    init_avatar();
    init_site();
    init_theme();
    ...
    init_i18n();
    init_session();
    var markdownList = byId("markdownList");
    var markdownLoadMoreBtn = byId("markdownLoadMoreBtn");
    var nextOffset = 0;
    var hasMore = true;
    initStoredTheme();
    bindThemeSync();
    ...
    function createMarkdownCard(entry) {
      ...
      const cover = entry.cover_url ? `<img class="markdown-card-cover" src="${buildAssetUrl(entry.cover_url)}" alt="${escapeHtml(entry.title)}" loading="lazy" />` : "";
      const summary = entry.summary ? `<div class="markdown-card-summary">${escapeHtml(entry.summary)}</div>` : "";
      card.innerHTML = `
...
export default require_markdowns();
