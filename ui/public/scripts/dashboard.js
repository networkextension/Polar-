...
async function fetchLoginHistory(limit = 5) {
  return requestJson(`/api/login-history?limit=${limit}`);
}
async function fetchEntries(offset, limit = 10) {
...
async function fetchAvailableLLMConfigs() {
  return requestJson("/api/llm-configs/available");
}
async function createLLMConfig(payload) {
  return requestJson("/api/llm-configs", {
    method: "POST",
    body: payload
  });
...
async function updateLLMConfig(id, payload) {
  return requestJson(`/api/llm-configs/${id}`, {
    method: "PUT",
    body: payload
  });
}
async function removeLLMConfig(id) {
  return requestJson(`/api/llm-configs/${id}`, {
    method: "DELETE"
  });
}
...
async function createBotUser(payload) {
  return requestJson("/api/bots", {
    method: "POST",
    body: payload
  });
}
async function updateBotUser(id, payload) {
  return requestJson(`/api/bots/${id}`, {
    method: "PUT",
    body: payload
  });
}
async function removeBotUser(id) {
  return requestJson(`/api/bots/${id}`, {
    method: "DELETE"
  });
}
...
async function fetchInviteCodes(limit = 30) {
...
async function uploadSiteIcon(formData) {
  return requestJson("/api/site-settings/icon", {
    method: "POST",
    body: formData
  });
}
async function uploadApplePushCertificate(environment, formData) {
  return requestJson(`/api/site-settings/apple-push-cert?env=${environment}`, {
    method: "POST",
    body: formData
  });
}
async function deleteApplePushCertificate(environment) {
  return requestJson(`/api/site-settings/apple-push-cert?env=${environment}`, {
    method: "DELETE"
  });
}
...
async function uploadUserIcon(formData) {
...
async function finishPasskeyRegistration(sessionId, payload) {
...
// src/lib/client.ts
function formatDeviceType(deviceType, tFn) {
  switch (deviceType) {
    case "ios":
      return "iOS";
    case "android":
...
function slugify(text) {
  return text.toLowerCase().trim().replace(/[\s]+/g, "-").replace(/[^\p{L}\p{N}\-_]+/gu, "").replace(/-+/g, "-").replace(/^-|-$/g, "");
}
function configureMarked() {
  ...
  marked.use({
    ...
    renderer: {
      ...
      link(hrefOrToken, titleOrUndefined, textOrUndefined) {
        let href = "";
        let title = null;
        let text = "";
      ...
      heading(textOrToken, levelMaybe) {
        let text = "";
        let level = 1;
        let raw = "";
...
// src/lib/llm_presets.ts
function getPresetByID(id) {
  return LLM_PROVIDER_PRESETS.find((item) => item.id === id);
}
function resolvePresetEndpoint(preset) {
  if (preset.endpoint) {
...
function matchPresetByBaseURL(baseURL) {
...
var require_dashboard = __commonJS({
  "src/dashboard.ts"() {
    ...
    init_theme();
    init_i18n();
    var welcomeText = byId("welcomeText");
    var entryList = byId("entryList");
    var entryContent = byId("entryContent");
    var logoutBtn = byId("logoutBtn");
    var newEntryBtn = byId("newEntryBtn");
    var loadMoreBtn = byId("loadMoreBtn");
    var editBtn = byId("editBtn");
    var deleteBtn = byId("deleteBtn");
    var drawerToggleBtn = byId("drawerToggleBtn");
    var drawerCloseBtn = byId("drawerCloseBtn");
    var drawerBackdrop = byId("drawerBackdrop");
    var entryDrawer = byId("entryDrawer");
    var loginHistoryList = byId("loginHistoryList");
    var themeToggleBtn = byId("themeToggleBtn");
    var languageToggleBtn = byId("languageToggleBtn");
    var languageCurrentValue = byId("languageCurrentValue");
    var passkeyRegisterBtn = byId("passkeyRegisterBtn");
    var passkeyStatus = byId("passkeyStatus");
    var passkeyList = byId("passkeyList");
    var sendEmailVerificationBtn = byId("sendEmailVerificationBtn");
    var emailVerificationAddress = byId("emailVerificationAddress");
    var emailVerificationState = byId("emailVerificationState");
    var emailVerificationStatus = byId("emailVerificationStatus");
    var userIcon = byId("userIcon");
    var iconFile = byId("iconFile");
    var iconEditor = byId("iconEditor");
    var iconCanvas = byId("iconCanvas");
    var iconZoom = byId("iconZoom");
    var saveIconBtn = byId("saveIconBtn");
    var cancelIconBtn = byId("cancelIconBtn");
    var iconStatus = byId("iconStatus");
    var settingsCardAvatar = byId("settingsCardAvatar");
    var settingsCardName = byId("settingsCardName");
    var settingsCardMeta = byId("settingsCardMeta");
    var dashboardRoleBadge = byId("dashboardRoleBadge");
    var settingsCardRoleBadge = byId("settingsCardRoleBadge");
    var settingsProfileName = byId("settingsProfileName");
    var settingsProfileMeta = byId("settingsProfileMeta");
    var settingsProfileRoleBadge = byId("settingsProfileRoleBadge");
    var addTagBtn = byId("addTagBtn");
    var tagModal = byId("tagModal");
    var tagModalTitle = byId("tagModalTitle");
    var tagModalCloseBtn = byId("tagModalCloseBtn");
    var tagForm = byId("tagForm");
    var tagName = byId("tagName");
    var tagSlug = byId("tagSlug");
    var tagDesc = byId("tagDesc");
    var tagOrder = byId("tagOrder");
    var tagFormStatus = byId("tagFormStatus");
    var tagSubmitBtn = byId("tagSubmitBtn");
    var openSiteAdminBtn = byId("openSiteAdminBtn");
    var siteAdminModal = byId("siteAdminModal");
    var siteAdminModalTitle = byId("siteAdminModalTitle");
    var siteAdminModalCloseBtn = byId("siteAdminModalCloseBtn");
    var settingsSectionLead = byId("settingsSectionLead");
    var siteAdminPanel = byId("siteAdminPanel");
    var themeCurrentValue = byId("themeCurrentValue");
    var llmConfigForm = byId("llmConfigForm");
    var llmProviderPresetSelect = byId("llmProviderPresetSelect");
    var llmProviderPresetNote = byId("llmProviderPresetNote");
    var llmProviderPresetDocs = byId("llmProviderPresetDocs");
    var llmConfigNameInput = byId("llmConfigNameInput");
    var llmConfigBaseUrlInput = byId("llmConfigBaseUrlInput");
    var llmConfigModelInput = byId("llmConfigModelInput");
    var llmConfigApiKeyInput = byId("llmConfigApiKeyInput");
    var llmConfigSystemPromptInput = byId("llmConfigSystemPromptInput");
    var llmConfigSharedInput = byId("llmConfigSharedInput");
    var llmConfigResetBtn = byId("llmConfigResetBtn");
    var llmConfigTestBtn = byId("llmConfigTestBtn");
    var llmConfigSubmitBtn = byId("llmConfigSubmitBtn");
    var llmConfigStatus = byId("llmConfigStatus");
    var llmConfigList = byId("llmConfigList");
    var settingsGitTagVersion = byId("settingsGitTagVersion");
    var settingsOS = byId("settingsOS");
    var settingsCPUArch = byId("settingsCPUArch");
    var settingsPartitionCapacity = byId("settingsPartitionCapacity");
    var settingsPartitionPath = byId("settingsPartitionPath");
    var botUserForm = byId("botUserForm");
    var botUserNameInput = byId("botUserNameInput");
    var botUserConfigSelect = byId("botUserConfigSelect");
    var botUserDescriptionInput = byId("botUserDescriptionInput");
    var botUserSystemPromptInput = byId("botUserSystemPromptInput");
    var botUserResetBtn = byId("botUserResetBtn");
    var botUserSubmitBtn = byId("botUserSubmitBtn");
    var botUserStatus = byId("botUserStatus");
    var botUserList = byId("botUserList");
    var siteNameInput = byId("siteNameInput");
    var siteDescriptionInput = byId("siteDescriptionInput");
    var siteRegistrationInviteRequired = byId("siteRegistrationInviteRequired");
    var saveSiteBtn = byId("saveSiteBtn");
    var siteStatus = byId("siteStatus");
    var inviteCodeCountInput = byId("inviteCodeCountInput");
    var generateInviteCodeBtn = byId("generateInviteCodeBtn");
    var inviteCodeStatus = byId("inviteCodeStatus");
    var inviteCodeList = byId("inviteCodeList");
    var siteIconPreview = byId("siteIconPreview");
    var siteIconFile = byId("siteIconFile");
    var applePushDevFile = byId("applePushDevFile");
    var applePushProdFile = byId("applePushProdFile");
    var applePushDevMeta = byId("applePushDevMeta");
    var applePushProdMeta = byId("applePushProdMeta");
    var applePushDevDeleteBtn = byId("applePushDevDeleteBtn");
    var applePushProdDeleteBtn = byId("applePushProdDeleteBtn");
    var siteAddTagBtnProxy = byId("siteAddTagBtnProxy");
    var tagList = byId("tagList");
    var settingsNavButtons = Array.from(document.querySelectorAll("[data-settings-nav]"));
    var settingsPanels = Array.from(document.querySelectorAll("[data-settings-panel]"));
    var settingsOpenButtons = Array.from(document.querySelectorAll("[data-open-settings-center], #openSiteAdminBtn"));
    ...
    var currentTags = [];
    var editingLLMConfigId = null;
    var editingBotUserId = null;
    var currentLLMConfigs = [];
    var currentAvailableLLMConfigs = [];
    var currentBotUsers = [];
    var currentInviteCodes = [];
    var activeSettingsSection = "personalization";
    function populateLLMProviderPresets() {
      llmProviderPresetSelect.innerHTML = LLM_PROVIDER_PRESETS.map((item) => `<option value="${item.id}">${item.displayName}</option>`).join("");
    }
    ...
    function renderPasskeys(credentials = []) {
      ...
      passkeyList.innerHTML = credentials.map((item) => {
        const createdAt = new Date(item.created_at).toLocaleString();
        const updatedAt = new Date(item.updated_at).toLocaleString();
        return `
        <li>
          <div class="meta-title">\u5DF2\u7ED1\u5B9A \xB7 ${formatPasskeyLabel(item.credential_id)}</div>
    ...
    function renderSystemInfo(systemInfo) {
      ...
      settingsPartitionPath.textContent = systemInfo?.partition_path ? `\u8DEF\u5F84\uFF1A${systemInfo.partition_path}` : "";
    }
    ...
    function renderLLMConfigList(configs) {
      ...
      llmConfigList.innerHTML = configs.map(
        (config) => `
    ...
    function syncBotConfigOptions(configs) {
      ...
      botUserConfigSelect.innerHTML = configs.map((config) => `<option value="${config.id}">${config.name} \xB7 ${config.model}</option>`).join("");
    ...
    function renderBotUserList(bots) {
      ...
      botUserList.innerHTML = bots.map(
        (bot) => `
    ...
    async function loadLoginHistory() {
      ...
      loginHistoryList.innerHTML = records.map((record) => {
        const time = new Date(record.logged_in_at).toLocaleString();
        return `
        <li>
          <div class="meta-title">${record.ip_address || t("dashboard.unknownIp")} \xB7 ${formatLoginMethod(record.login_method)} \xB7 ${formatDeviceType(record.device_type, t)}</div>
    ...
    async function loadSiteAdminData() {
      const tasks = [
        ...
        (async () => {
          if (isAdmin) {
            const [ownResult, availableResult2] = await Promise.all([fetchLLMConfigs(), fetchAvailableLLMConfigs()]);
            ...
            if (availableResult2.response.ok) {
              currentAvailableLLMConfigs = availableResult2.data.configs || [];
              syncBotConfigOptions(currentAvailableLLMConfigs);
            } else {
              currentAvailableLLMConfigs = [];
              botUserStatus.textContent = availableResult2.data.error || t("dashboard.availableConfigLoadFailed");
              syncBotConfigOptions(currentAvailableLLMConfigs);
            }
            return;
          }
    ...
    async function loadEntries(reset = false) {
      ...
      entries.forEach((entry) => {
        ...
        li.innerHTML = entry.is_public ? `<span>${entry.title}</span><span class="tag-chip">Public</span>` : `<span>${entry.title}</span>`;
    ...
    tagForm.addEventListener("submit", async (event) => {
      ...
      try {
        const { response, data } = editingTagId ? await updateTag(editingTagId, payload) : await createTag(payload);
        if (!response.ok) {
    ...
    llmConfigForm.addEventListener("submit", async (event) => {
      ...
      try {
        const { response, data } = editingLLMConfigId ? await updateLLMConfig(editingLLMConfigId, {
          ...payload,
    ...
    botUserForm.addEventListener("submit", async (event) => {
      ...
      try {
        const { response, data } = editingBotUserId ? await updateBotUser(editingBotUserId, payload) : await createBotUser(payload);
        if (!response.ok) {
    ...
    var initialTheme = initStoredTheme();
