...
async function updateMyProfile(bio) {
  return requestJson("/api/users/me/profile", {
    method: "PUT",
    body: { bio }
  });
}
async function upsertRecommendation(userId, content) {
...
async function fetchFollowers(userId, limit = 20, offset = 0) {
  const params = new URLSearchParams({ limit: String(limit), offset: String(offset) });
  return requestJson(
    `/api/users/${encodeURIComponent(userId)}/followers?${params.toString()}`
  );
}
async function fetchFollowing(userId, limit = 20, offset = 0) {
  const params = new URLSearchParams({ limit: String(limit), offset: String(offset) });
  return requestJson(
...
var require_profile = __commonJS({
  "src/profile.ts"() {
    ...
    init_theme();
    init_i18n();
    var profileWelcome = byId("profileWelcome");
    var profileCard = byId("profileCard");
    var profileBioPanel = byId("profileBioPanel");
    var profileFollowPanel = byId("profileFollowPanel");
    var profileRecommendationPanel = byId("profileRecommendationPanel");
    var currentUserId = "";
    var currentUserEmail = "";
    var currentUserEmailVerified = false;
    var profileUserId = "";
    var followTab = "followers";
    var followCurrentProfile = null;
    initStoredTheme();
    bindThemeSync();
    function getUserId() {
    ...
    function renderProfileCard(profile) {
      ...
      const emailLine = profile.is_me && profile.email ? `<div class="profile-meta-line profile-email-line"><span class="profile-meta-label">${t("profile.email")}</span><a class="profile-email-link" href="mailto:${escapeHtml(profile.email)}">${escapeHtml(profile.email)}</a></div>` : "";
      const canMessage = !profile.is_me && !profile.i_blocked_user && !profile.blocked_me;
      const blockMessage = profile.i_blocked_user ? t("profile.youBlockedUser") : profile.blocked_me ? t("profile.userBlockedYou") : "";
      const messageAction = canMessage ? `<a class="btn-inline btn-secondary" href="/chat.html?user_id=${encodeURIComponent(profile.user_id)}&username=${encodeURIComponent(profile.username)}">${t("profile.sendMessage")}</a>` : "";
      const canFollow = !profile.is_me && !profile.i_blocked_user && !profile.blocked_me;
      const followAction = canFollow ? `<button id="profileFollowBtn" class="btn-inline ${profile.is_following ? "btn-secondary" : "btn-primary"}" type="button">${profile.is_following ? t("profile.unfollowUser") : t("profile.followUser")}</button>` : "";
      const blockAction = !profile.is_me ? `<button id="profileBlockBtn" class="btn-inline btn-secondary" type="button">${profile.i_blocked_user ? t("profile.unblockUser") : t("profile.blockUser")}</button>` : "";
  ...
  `;
      const statusLine = blockMessage ? `<div class="status-text">${escapeHtml(blockMessage)}</div>` : "";
  ...
  `;
      ...
      followBtn?.addEventListener("click", async () => {
        ...
        const result = profile.is_following ? await unfollowUser(profile.user_id) : await followUser(profile.user_id);
  ...
  `;
    ...
    function renderBioPanel(profile) {
      if (profile.is_me) {
        ...
        const verificationAction = currentUserEmailVerified ? "" : `<button id="profileSendVerificationBtn" class="btn-inline btn-secondary" type="button">${t("profile.sendVerificationEmail")}</button>`;
  ...
  `;
    ...
    function renderRecommendationPanel(profile) {
      ...
      const formHtml = profile.can_recommend && !profile.i_blocked_user && !profile.blocked_me ? `
    ...
    ` : "";
      const listHtml = recommendations.length ? recommendations.map((item) => {
        ...
        return `
          ...
          `;
      }).join("") : `<div class='reply-empty'>${t("profile.noRecommendations")}</div>`;
  ...
  `;
    ...
    async function loadProfile() {
      ...
      profileWelcome.textContent = profile.is_me ? t("profile.completeProfile") : t("profile.viewingProfile", { username: profile.username });
