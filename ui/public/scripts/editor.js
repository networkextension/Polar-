...
async function assistMarkdownWithBot(payload) {
...
var require_editor = __commonJS({
  "src/editor.ts"() {
    ...
    init_dashboard();
    var API_BASE2 = "";
    var alertBox = byId("alert");
    var titleInput = byId("titleInput");
    var contentInput = byId("contentInput");
    var preview = byId("preview");
    var saveBtn = byId("saveBtn");
    var backBtn = byId("backBtn");
    var welcomeText = byId("welcomeText");
    var publicToggle = byId("publicToggle");
    var publicHint = byId("publicHint");
    var assistBotSelect = byId("assistBotSelect");
    var assistLLMSelect = byId("assistLLMSelect");
    var assistInstructionInput = byId("assistInstructionInput");
    var assistRunBtn = byId("assistRunBtn");
    var assistApplyBtn = byId("assistApplyBtn");
    var assistDiffBtn = byId("assistDiffBtn");
    var assistStatus = byId("assistStatus");
    var assistPreview = byId("assistPreview");
    var assistDiff = byId("assistDiff");
    var entryId = new URLSearchParams(window.location.search).get("id");
    var canEdit = true;
    var assistResultContent = "";
    var assistSourceContent = "";
    var assistDiffVisible = false;
    var currentBots = [];
    var currentLLMConfigs = [];
    initStoredTheme();
    bindThemeSync();
    async function ensureLogin() {
      const res = await fetch(`${API_BASE2}/api/me`, { credentials: "include" });
      if (!res.ok) {
    ...
    function updatePublicHint() {
      if (!canEdit) {
        publicHint.textContent = publicToggle.checked ? t("editor.publicReadOnly", { url: getPublicUrl() }) : t("editor.readOnly");
        return;
      }
      ...
      publicHint.textContent = entryId ? t("editor.publicUrl", { url: getPublicUrl() }) : t("editor.publicAfterSave");
    }
    ...
    function renderAssistOptions() {
      if (!currentBots.length) {
        ...
        assistBotSelect.innerHTML = currentBots.map((bot) => `<option value="${bot.id}">${bot.name} \xB7 ${bot.config_name || "Default"}</option>`).join("");
        assistRunBtn.disabled = !canEdit;
      }
      const llmOptions = currentLLMConfigs.map((cfg) => `<option value="${cfg.id}">${cfg.name} \xB7 ${cfg.model}</option>`).join("");
    ...
    async function loadEntry() {
      ...
      const res = await fetch(`${API_BASE2}/api/markdown/${entryId}`, {
        credentials: "include"
      });
      if (!res.ok) {
    ...
    assistRunBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const { response, data } = await assistMarkdownWithBot({
          bot_id: botID,
          llm_config_id: selectedLLMID > 0 ? selectedLLMID : void 0,
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      try {
        const targetUrl = entryId ? `${API_BASE2}/api/markdown/${entryId}` : `${API_BASE2}/api/markdown`;
        ...
        alertBox.className = "alert success";
        alertBox.textContent = entryId ? t("editor.updateSuccess") : t("editor.saveSuccess", { id: String(data.id) });
