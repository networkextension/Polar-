...
async function assistPostWithBot(payload) {
  return requestJson("/api/posts/assist-with-bot", {
    method: "POST",
    body: payload
  });
}
async function assistReplyWithBot(postId, payload) {
...
var require_post = __commonJS({
  "src/post.ts"() {
    ...
    var postDetail = byId("postDetail");
    var postForm = byId("postForm");
    var postContent = byId("postContent");
    var postType = byId("postType");
    var postTag = byId("postTag");
    var taskFields = byId("taskFields");
    var taskLocation = byId("taskLocation");
    var taskStartAt = byId("taskStartAt");
    var taskEndAt = byId("taskEndAt");
    var workingHours = byId("workingHours");
    var applyDeadline = byId("applyDeadline");
    var postImages = byId("postImages");
    var postVideos = byId("postVideos");
    var postFormStatus = byId("postFormStatus");
    var postSubmitBtn = byId("postSubmitBtn");
    var postAssistBotSelect = byId("postAssistBot");
    var postAssistLLMSelect = byId("postAssistLLM");
    var postAssistTopicInput = byId("postAssistTopic");
    var postAssistInstructionInput = byId("postAssistInstruction");
    var postAssistStatus = byId("postAssistStatus");
    var postAssistPreview = byId("postAssistPreview");
    var postAssistRunBtn = byId("postAssistRunBtn");
    var postAssistApplyBtn = byId("postAssistApplyBtn");
    var currentUserId = "";
    var currentUserRole = "user";
    var assistBots = [];
    var assistLLMConfigs = [];
    var assistBotsLoaded = false;
    var postAssistResult = "";
    var replyAssistResult = "";
    var videoModal = null;
    ...
    function renderPostAssistOptions() {
      if (!assistBots.length) {
        ...
        postAssistBotSelect.innerHTML = assistBots.map((bot) => `<option value="${bot.id}">${bot.name}${bot.config_name ? ` \xB7 ${bot.config_name}` : ""}</option>`).join("");
        postAssistRunBtn.disabled = false;
      }
      const llmOptions = assistLLMConfigs.map((cfg) => `<option value="${cfg.id}">${cfg.name} \xB7 ${cfg.model}</option>`).join("");
      ...
      postAssistRunBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          const { response, data } = await assistPostWithBot({
            bot_id: botId,
            llm_config_id: llmId > 0 ? llmId : void 0,
            content: draft,
            topic,
          ...
          const llmLabel = data.llm?.model ? ` \xB7 ${data.llm.model}` : "";
    ...
    function renderReplyAssistBotOptions(select, llmSelect) {
      if (!assistBots.length) {
        ...
        select.innerHTML = assistBots.map((bot) => `<option value="${bot.id}">${bot.name}${bot.config_name ? ` \xB7 ${bot.config_name}` : ""}</option>`).join("");
      }
      const llmOptions = assistLLMConfigs.map((cfg) => `<option value="${cfg.id}">${cfg.name} \xB7 ${cfg.model}</option>`).join("");
  ...
  `;
    ...
    async function loadReplies(postId) {
      ...
      const res = await fetch(`${API_BASE3}/api/posts/${postId}/replies?limit=50`, {
        credentials: "include"
      });
      if (!res.ok) {
      ...
      replyList.innerHTML = replies.map((reply) => {
        ...
        <div class="reply-item">
          ...
          <div class="reply-body">
            <div class="reply-meta">${reply.username} \xB7 ${formatTime(reply.created_at)}</div>
    ...
    function renderPost(post) {
      ...
      const images = postImagePreviews.map((url) => `<img src="${url}" alt="post image" loading="lazy" />`).join("");
      ...
      <div class="task-meta-card">
        ...
        <div class="task-meta-grid">
          ...
          ${post.task.selected_applicant_name ? `<div><strong>${t("post.selectedApplicant")}</strong>${escapeHtml(post.task.selected_applicant_name)}</div>` : ""}
        </div>
      </div>
    ` : "";
      ...
      <div class="task-actions">
        ${post.task.can_apply ? `<button id="taskApplyBtn" class="btn-inline btn-secondary" type="button">${post.task.applied_by_me ? t("post.withdrawApplication") : t("post.applyTask")}</button>` : ""}
        ${post.task.can_manage && post.task.application_status === "open" ? `<button id="taskCloseBtn" class="btn-inline btn-secondary" type="button">${t("post.closeApplications")}</button>
    ...
    <div class="post-actions">
      <button id="detailLikeBtn" class="btn-inline btn-secondary" type="button">
        ${post.liked_by_me ? t("post.liked") : t("post.like")} \xB7 ${post.like_count}
      </button>
  ...
  `;
      ...
      likeBtn.addEventListener("click", async () => {
        ...
        likeBtn.textContent = `${post.liked_by_me ? t("post.liked") : t("post.like")} \xB7 ${post.like_count}`;
      });
      ...
      replyForm.addEventListener("submit", async (event) => {
        ...
        const res = await fetch(`${API_BASE3}/api/posts/${post.id}/replies`, {
          method: "POST",
      ...
      taskApplyBtn?.addEventListener("click", async () => {
        ...
        const res = await fetch(`${API_BASE3}/api/tasks/${post.id}/apply`, {
          method,
          credentials: "include"
        });
        if (!res.ok) {
      ...
      taskCloseBtn?.addEventListener("click", async () => {
        const res = await fetch(`${API_BASE3}/api/tasks/${post.id}/close`, {
          method: "POST",
          credentials: "include"
        });
        if (!res.ok) {
      ...
      taskLoadApplicantsBtn?.addEventListener("click", async () => {
        ...
        const res = await fetch(`${API_BASE3}/api/tasks/${post.id}/applications`, {
          credentials: "include"
        });
        if (!res.ok) {
        ...
        taskApplicantsPanel.innerHTML = applications.map((item) => {
          const avatarUrl = resolveAvatar(item.username, item.user_icon, 40);
          const defaultTemplate = post.task?.invitation_template || t("post.invitationDefault", { content: post.content });
          return `
          <div class="task-applicant-item">
            <div class="task-applicant-head">
              ...
              <div>
                <div class="reply-meta"><a class="post-author-name" href="${profileUrl(item.user_id)}">${item.username}</a> \xB7 ${formatTime(item.applied_at)}</div>
        ...
        taskApplicantsPanel.querySelectorAll(".task-select-btn").forEach((button) => {
          button.addEventListener("click", async () => {
            ...
            const textarea = taskApplicantsPanel.querySelector(
              `.task-template-input[data-user-id="${applicantUserId}"]`
            );
            const messageTemplate = textarea?.value.trim() || "";
            button.disabled = true;
            const selectRes = await fetch(`${API_BASE3}/api/tasks/${post.id}/select-candidate`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              credentials: "include",
              body: JSON.stringify({
      ...
      taskResultForm?.addEventListener("submit", async (event) => {
        ...
        const res = await fetch(`${API_BASE3}/api/tasks/${post.id}/results`, {
          method: "POST",
          credentials: "include",
          body: formData
        });
    ...
    async function loadTaskResults(postId) {
      ...
      const res = await fetch(`${API_BASE3}/api/tasks/${postId}/results`, {
        credentials: "include"
      });
      if (!res.ok) {
      ...
      container.innerHTML = results.map((result) => {
        ...
        const images = (result.images || []).map((url) => `<img src="${buildAssetUrl(url)}" alt="task result image" />`).join("");
        const videos = (result.video_items || []).map((item) => {
        ...
        <div class="task-result-item">
          <div class="task-applicant-head">
            ...
            <div class="reply-meta"><a class="post-author-name" href="${profileUrl(result.user_id)}">${result.username}</a> \xB7 ${formatTime(result.created_at)}</div>
          </div>
    ...
    async function loadPost() {
      ...
      const res = await fetch(`${API_BASE3}/api/posts/${postId}`, {
        credentials: "include"
      });
      if (!res.ok) {
    ...
    postForm.addEventListener("submit", async (event) => {
      ...
      try {
        const res = await fetch(`${API_BASE3}/api/posts`, {
          method: "POST",
          credentials: "include",
          body: formData
        });
