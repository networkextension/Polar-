...
var require_bots = __commonJS({
  "src/bots.ts"() {
    ...
    var pendingDeleteLLMConfigId = null;
    var pendingDeleteBotId = null;
    var expandedBotId = null;
    ...
    function renderLLMConfigList(configs) {
      ...
      llmConfigList.innerHTML = configs.map((config) => {
        const isPending = pendingDeleteLLMConfigId === config.id;
        const actions = isPending ? `<button class="btn-inline" type="button" data-action="confirm-delete">${t("common.confirmDelete")}</button>
           <button class="btn-inline btn-secondary" type="button" data-action="cancel-delete">${t("common.cancel")}</button>` : `<button class="btn-inline btn-secondary" type="button" data-action="edit">${t("common.edit")}</button>
    ...
    function renderBotUserList(bots) {
      ...
      botUserList.innerHTML = bots.map((bot) => {
        const isPending = pendingDeleteBotId === bot.id;
        const isExpanded = expandedBotId === bot.id;
        const actions = isPending ? `<button class="btn-inline" type="button" data-action="confirm-delete">${t("common.confirmDelete")}</button>
           <button class="btn-inline btn-secondary" type="button" data-action="cancel-delete">${t("common.cancel")}</button>` : `<button class="btn-inline btn-secondary" type="button" data-action="toggle-expand">${isExpanded ? "\u25B2" : "\u25BC"}</button>
    ...
    llmConfigTestBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        llmConfigStatus.textContent = response.ok ? data.message || t("dashboard.llmTestSuccess") : data.error || t("dashboard.llmTestFailed");
      } catch {
    ...
    llmConfigForm.addEventListener("submit", async (event) => {
      ...
      try {
        const { response, data } = editingLLMConfigId ? await updateLLMConfig(editingLLMConfigId, { ...payload, update_api_key: payload.api_key !== "" }) : await createLLMConfig(payload);
        if (!response.ok) {
