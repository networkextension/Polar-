...
var require_latch = __commonJS({
  "src/latch.ts"() {
    ...
    init_dom();
    init_session();
    var lpOverlay = byId("lpOverlay");
    var latchWelcome = byId("latchWelcome");
    var latchTabProxies = byId("latchTabProxies");
    var latchTabRules = byId("latchTabRules");
    var latchSubtabBtns = document.querySelectorAll("[data-latch-tab]");
    var latchTabPanels = document.querySelectorAll("[data-latch-panel]");
    var lpNavBtns = document.querySelectorAll("[data-lp-nav]");
    var latchProxyStatus = byId("latchProxyStatus");
    var latchProxyList = byId("latchProxyList");
    var lpAddProxyBtn = byId("lpAddProxyBtn");
    var lpProxySearch = byId("lpProxySearch");
    var latchRuleStatus = byId("latchRuleStatus");
    var latchRuleList = byId("latchRuleList");
    var lpAddRuleBtn = byId("lpAddRuleBtn");
    var lpRuleSearch = byId("lpRuleSearch");
    var latchProfileAdminGrid = byId("latchProfileAdminGrid");
    var latchProfileStatus = byId("latchProfileStatus");
    var latchProfileList = byId("latchProfileList");
    var lpAddProfileBtn = byId("lpAddProfileBtn");
    var latchProfileUserView = byId("latchProfileUserView");
    var latchProfileUserList = byId("latchProfileUserList");
    var lpGoRules = byId("lpGoRules");
    var lpGoRulesAlt = byId("lpGoRulesAlt");
    var lpGoProfiles = byId("lpGoProfiles");
    var lpProxyPanel = byId("lpProxyPanel");
    var lpProxyClose = byId("lpProxyClose");
    var latchProxyFormTitle = byId("latchProxyFormTitle");
    var latchProxyNameInput = byId("latchProxyNameInput");
    var latchProxyTypeSelect = byId("latchProxyTypeSelect");
    var latchProxyConfigInput = byId("latchProxyConfigInput");
    var latchProxyResetBtn = byId("latchProxyResetBtn");
    var latchProxySubmitBtn = byId("latchProxySubmitBtn");
    var lpRulePanel = byId("lpRulePanel");
    var lpRuleClose = byId("lpRuleClose");
    var latchRuleFormTitle = byId("latchRuleFormTitle");
    var latchRuleNameInput = byId("latchRuleNameInput");
    var latchRuleSourceInlineBtn = byId("latchRuleSourceInlineBtn");
    var latchRuleSourceFileBtn = byId("latchRuleSourceFileBtn");
    var latchRuleInlineSection = byId("latchRuleInlineSection");
    var latchRuleFileSection = byId("latchRuleFileSection");
    var latchRuleContentInput = byId("latchRuleContentInput");
    var latchRuleFileInput = byId("latchRuleFileInput");
    var latchRuleUploadBtn = byId("latchRuleUploadBtn");
    var latchRuleResetBtn = byId("latchRuleResetBtn");
    var latchRuleSubmitBtn = byId("latchRuleSubmitBtn");
    var lpProfilePanel = byId("lpProfilePanel");
    var lpProfileClose = byId("lpProfileClose");
    var latchProfileFormTitle = byId("latchProfileFormTitle");
    var latchProfileNameInput = byId("latchProfileNameInput");
    var latchProfileDescInput = byId("latchProfileDescInput");
    var latchProfileEnabledInput = byId("latchProfileEnabledInput");
    var latchProfileShareableInput = byId("latchProfileShareableInput");
    var latchProfileProxyCheckboxes = byId("latchProfileProxyCheckboxes");
    var latchProfileRuleRadios = byId("latchProfileRuleRadios");
    var latchProfileResetBtn = byId("latchProfileResetBtn");
    var latchProfileSubmitBtn = byId("latchProfileSubmitBtn");
    var isAdmin = false;
    var editingProxyGroupId = null;
    var editingRuleGroupId = null;
    var editingProfileId = null;
    var currentLatchProxies = [];
    var currentLatchRules = [];
    var currentLatchProfiles = [];
    ...
    function switchTab(tab) {
      ...
      latchTabPanels.forEach((panel) => {
        panel.hidden = panel.dataset.latchPanel !== tab;
      });
    ...
    function syncProfileSelectors(proxies, rules) {
      latchProfileProxyCheckboxes.innerHTML = proxies.length ? proxies.map((p) => `
        ...
        </label>`).join("") : '<span style="color:var(--muted,#aaa);font-size:13px;padding:4px;">\u6682\u65E0\u4EE3\u7406</span>';
    ...
    function renderAdminProfiles(profiles, proxies, rules) {
      ...
      latchProfileList.innerHTML = profiles.map((prof) => {
        const chips = prof.proxy_group_ids.map((gid) => proxyMap.get(gid)).filter(Boolean).map((p) => `<span class="lp-proxy-chip">${p.name}</span>`).join("") || `<span style="color:#bbb;font-size:12px;">\u2014</span>`;
        const ruleLabel = prof.rule_group_id && ruleMap.get(prof.rule_group_id) ? `<span class="lp-ver">${ruleMap.get(prof.rule_group_id).name}</span>` : `<span style="color:#bbb;font-size:12px;">\u2014</span>`;
        return `
    ...
    function renderUserProfiles(profiles) {
      ...
      latchProfileUserList.innerHTML = profiles.map((prof) => {
        const chips = (prof.proxies || []).map((p) => `<span class="lp-proxy-chip">${p.name} <span style="opacity:.6;">${p.type}</span></span>`).join("") || `<span style="color:#bbb;font-size:12px;">\u65E0\u4EE3\u7406</span>`;
        const ruleLabel = prof.rule ? `<span class="lp-ver">${prof.rule.name} v${prof.rule.version}</span>` : `<span style="color:#bbb;font-size:12px;">\u65E0\u89C4\u5219</span>`;
        return `
    ...
    function wireAdminEvents() {
      ...
      latchProxySubmitBtn.addEventListener("click", async () => {
        ...
        try {
          const { response, data } = editingProxyGroupId ? await updateLatchProxy(editingProxyGroupId, { name, type, config }) : await createLatchProxy({ name, type, config });
          if (!response.ok) {
...
` + versions.map((v) => `v${v.version}  SHA1:${v.sha1.slice(0, 8)}  ${new Date(v.created_at).toLocaleString()}`).join("\n") + "\n\n\u8F93\u5165\u8981\u56DE\u6EDA\u5230\u7684\u7248\u672C\u53F7 (\u7559\u7A7A\u53D6\u6D88):"
      ...
      latchRuleSubmitBtn.addEventListener("click", async () => {
        ...
        try {
          const { response, data } = editingRuleGroupId ? await updateLatchRule(editingRuleGroupId, { name, content }) : await createLatchRule({ name, content });
          if (!response.ok) {
      ...
      latchRuleUploadBtn.addEventListener("click", async () => {
        ...
        try {
          const { response, data } = editingRuleGroupId ? await uploadLatchRuleFile(editingRuleGroupId, fd) : await createLatchRuleFromFile(fd);
          if (!response.ok) {
...
` + versions.map((v) => `v${v.version}  SHA1:${v.sha1.slice(0, 8)}  ${new Date(v.created_at).toLocaleString()}`).join("\n") + "\n\n\u8F93\u5165\u8981\u56DE\u6EDA\u5230\u7684\u7248\u672C\u53F7 (\u7559\u7A7A\u53D6\u6D88):"
      ...
      latchProfileSubmitBtn.addEventListener("click", async () => {
        ...
        const proxyGroupIds = Array.from(
          latchProfileProxyCheckboxes.querySelectorAll("input[type=checkbox]:checked")
        ).map((cb) => cb.value);
        ...
        try {
          const { response, data } = editingProfileId ? await updateLatchProfile(editingProfileId, payload) : await createLatchProfile(payload);
          if (!response.ok) {
