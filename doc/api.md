...
| 状态码 | 场景 |
| --- | --- |
| `400` | 请求字段校验失败（缺少字段、邮箱/密码格式不合法） |
| `403` | 站点启用邀请码注册，但未提供 `invitation_code` 或该邀请码无效/已被使用 |
| `409` | 邮箱已被注册 |
| `500` | 服务器内部错误 |
...
**POST** `/api/markdown/assist-with-bot`

权限要求：已登录用户，且 `bot_id` 必须属于当前用户

说明：

- 用于“文章润色/改写辅助”，不是整篇代写接口
- 服务端会要求模型尽量保留用户核心观点和事实，只做表达优化
- 返回结果可直接回填编辑器正文，避免去聊天页复制内容
...
```json
{
  ...
  "title": "周报草稿",
  "content": "这是我写的原始正文……",
  "instruction": "请润色成更专业的产品周报风格，语气克制，长度不要增加太多。"
}
```

字段说明：

- `bot_id`：必填，Bot 记录 ID（不是 `bot_user_id` 字符串）
- `llm_config_id`：可选，指定本次调用使用的 LLM 配置 ID（便于同一 Bot 下切换模型做对比）
  - 不传时，使用该 Bot 当前默认配置
  - 传值时，需是“当前用户可用”的配置（自己的或共享配置）
- `title`：可选，给模型作为上下文参考
- `content`：必填，当前编辑中的正文
- `instruction`：可选，本次润色要求（风格、语气、篇幅等）
